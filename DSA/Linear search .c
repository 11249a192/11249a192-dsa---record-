AIM:
To write and understand an algorithm that searches for a given element (key) in a list/array using the Linear Search technique, where each                                             element is sequentially compared with the key until it is found or the end of the list is reached.
...
Step 6: If A[i] == key,
         then,Print "Element found at position i"
           Go to Step 9
